  ...
  const fetchTransactions = async (newPage = 1, append = false) => {
    ...
    try {
      ...
      const res = await axiosClient.get(
        `transaction-logs/list-transactions?${query.toString()}`
      );
  ...
  const handleUpdate = async () => {
    try {
      await axiosClient.put(
        `/transaction-logs/update/${selectedTransaction?._id}`,
        selectedTransaction
      );
      const updated = transactions.map((t) =>
        t._id === selectedTransaction?._id ? selectedTransaction! : t
      );
        ...
        <Box sx={{ width: 320, p: 3 }}>
          ...
          <Stack spacing={2}>
            {filterFields.map((field, i) => (
              <FormControl fullWidth key={i} size="small">
