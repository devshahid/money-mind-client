...
const makeTransactionArb = (category: string, month: number, year: number, isCredit: boolean): fc.Arbitrary<ITransaction> =>
    fc.record({
        _id: fc.uuid(),
...
test("Property 22: Budget spend calculation — transactions from different months are excluded", () => {
    // Feature: money-mind-upgrade, Property 22: Budget spend calculation
    fc.assert(
        fc.property(categoryArb, monthArb, yearArb, fc.array(amountArb, { minLength: 1, maxLength: 10 }), (category, month, year, amounts) => {
