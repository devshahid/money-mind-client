...
const transactionArb = fc.record<ITransaction>({
    _id: fc.uuid(),
    transactionDate: fc.integer({ min: 2020, max: 2024 }).chain((year) => fc.integer({ min: 1, max: 12 }).map((month) => makeDateStr(month, year))),
...
test("Property 24: Category breakdown values sum to total debit amount for the period", () => {
    ...
    fc.assert(
        fc.property(fc.array(transactionArb, { minLength: 0, maxLength: 30 }), (transactions) => {
            ...
            const totalDebits = transactions.filter((tx) => !tx.isCredit).reduce((sum, tx) => sum + (parseFloat(tx.amount) || 0), 0);
            ...
            fc
                ...
                .chain((year) =>
                    fc
                        ...
                        .chain((startMonth) => fc.integer({ min: startMonth + 1, max: 12 }).map((endMonth) => ({ year, startMonth, endMonth }))),
                ),
