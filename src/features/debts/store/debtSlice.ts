...
import type { IAIDebtStrategy } from '../../ai-chat/types/ai'
...
export const createDebt = createAsyncThunk<
  IDebt,
  Omit<IDebt, '_id' | 'createdAt' | 'updatedAt'>,
  { rejectValue: string }
>('debts/createDebt', async (debt, { rejectWithValue }) => {
  try {
...
export const recordEMIPayment = createAsyncThunk<
  { payment: IEMIPayment; updatedDebt: IDebt },
  Omit<IEMIPayment, '_id'>,
  { rejectValue: string }
>('debts/recordEMIPayment', async (payment, { rejectWithValue }) => {
  try {
  ...
  extraReducers: builder => {
    ...
    builder.addCase(recordEMIPayment.fulfilled, (state, action) => {
      ...
      if (idx !== -1) {
        ...
        if (state.debts[idx].remainingBalance <= 0) {
...
export const {
  setAIDebtStrategy,
  clearDebtError,
  optimisticCreateDebt,
  optimisticUpdateDebt,
