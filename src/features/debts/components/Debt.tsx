...
export function DebtTable(): React.ReactElement {
  ...
  const fetchDebts = async (): Promise<void> => {
    ...
    try {
      ...
      const debtData = response.data.output.map((item: ApiResponse['output'][number]) => ({
        ...item.debtDetails,
        _id: item._id,
      ...
      {loading ? (
        ...
        <TableContainer sx={{ maxHeight: 440 }}>
          ...
          >
            ...
            <TableBody>
              {debts.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, index) => (
                <TableRow
                  hover
                  key={index}
                >
                  ...
                  <TableCell align='center'>{new Date(row.expectedEndDate).toLocaleDateString()}</TableCell>
