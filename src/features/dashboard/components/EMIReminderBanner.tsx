      ...
      {upcomingDebts.map(debt => {
        ...
        return (
          <Alert
            ...
            variant='outlined'
          >
            <AlertTitle>EMI Due Soon — {debt.debtName}</AlertTitle>₹{debt.monthlyExpectedEMI.toLocaleString()} due to{' '}
            {debt.lender} in {daysLeft === 0 ? 'today' : `${daysLeft} day${daysLeft > 1 ? 's' : ''}`}
          </Alert>
        )
      })}
    </Stack>
