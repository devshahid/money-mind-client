...
import { IAIGroupSuggestion } from "../types/ai";
...
/**
 * Check if a transaction is already in any group.
 * Used by components before dispatching createGroup / addToGroup.
 */
...
export const createGroup = createAsyncThunk<ITransactionGroup, { name: string; transactionIds: string[] }, { rejectValue: string }>(
    "transactionGroups/createGroup",
    async ({ name, transactionIds }, { rejectWithValue }) => {
        try {
...
export const addToGroup = createAsyncThunk<ITransactionGroup, { groupId: string; transactionId: string }, { rejectValue: string }>(
    "transactionGroups/addToGroup",
...
const transactionGroupSlice = createSlice({
    ...
    reducers: {
        /**
         * Optimistic create — push a temporary group into state before the server responds.
         * Enforces double-grouping prevention: if any transactionId already belongs to a group,
         * the optimistic entry is NOT added and the error field is set.
         */
        ...
        /** Revert an optimistic create on server failure */
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(createGroup.fulfilled, (state, action: PayloadAction<ITransactionGroup>) => {
            state.loading = false;
            // Replace optimistic entry if present, otherwise push
        ...
        builder.addCase(removeFromGroup.fulfilled, (state, action) => {
            ...
            if (idx !== -1) {
                state.groups[idx] = action.payload.group;
            }
        });
...
export const { optimisticCreateGroup, revertGroup, setAIGroupSuggestions, dismissGroupSuggestion, clearGroupError } = transactionGroupSlice.actions;
