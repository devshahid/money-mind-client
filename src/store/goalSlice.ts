...
export const listGoals = createAsyncThunk<IGoal[], void, { rejectValue: string }>("goals/listGoals", async (_, { rejectWithValue }) => {
    try {
...
export const deleteGoal = createAsyncThunk<string, string, { rejectValue: string }>("goals/deleteGoal", async (goalId, { rejectWithValue }) => {
    try {
...
const goalSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(recordContribution.fulfilled, (state, action) => {
            ...
            if (idx !== -1) {
                state.goals[idx] = updatedGoal;
            }

            // Push the contribution to contributions
...
export const { clearGoalError, optimisticCreateGoal, optimisticUpdateGoal, optimisticDeleteGoal, revertGoals } = goalSlice.actions;
