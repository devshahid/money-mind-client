...
import { IAISuggestion, IAIGroupSuggestion, IAIDebtStrategy, IAIChatMessage } from "../types/ai";
...
export const sendChatMessage = createAsyncThunk<IAIChatMessage, string, { rejectValue: string }>(
    ...
    async (content, { rejectWithValue }) => {
        try {
            const response = await axiosClient.post<{ data: IAIChatMessage }>("/ai/chat", { content });
...
export const fetchAnnotationSuggestions = createAsyncThunk<IAISuggestion[], string[], { rejectValue: string }>(
    ...
    async (transactionIds, { rejectWithValue }) => {
        try {
            const result = await aiServiceModule.fetchAnnotationSuggestions(transactionIds);
            return result;
        } catch {
